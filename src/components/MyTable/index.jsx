...
const MyTable = ({ columns, rows, fetchMore, totalCount }) => {
        ...
        <Table size="small" sx={{
          ...
          <TableBody>
            {rows.map((row, rowIndex) => (
              <TableRow key={row.id || rowIndex} sx={{'&:hover':{background:"#F4F6F8"}}}>
                {columns.map((col) => (
                  <TableCell
                    key={col.id}
                    align={col.align || 'left'}
                    sx={{
                      p: col.type === 'checkbox' || col.type?.includes('icon') ? 1 : 2,
                      width: col.type === 'checkbox' || col.type?.includes('icon') ? '1%' : 'auto',
                      whiteSpace: 'nowrap',
                      ...(col.sx || {})
                    }}
                  >
